...
def render_kanji(kanji, fontpath, fsize, angle, image_size: int):
    color_text = (0, 0, 0, randint(128, 255))
    color_bg = (255, 255, 255, randint(128, 255))
    bgsize = randint(fsize, image_size)
    off_y = fsize * 0.3
    circle_min = image_size//2 - bgsize//2
    circle_max = image_size//2 + bgsize//2
    ...
    text_pos = image_size//2 - fsize//2
    draw.text((text_pos, text_pos-off_y), kanji, color_text, font=font)
    return img.rotate(angle, resample=Image.BILINEAR)


def render_clutter(image_size: int):
    img = Image.new("RGBA", (image_size, image_size))
    draw = ImageDraw.Draw(img)
    for _ in range(randint(0, 100)):
        x, y = randint(-image_size, image_size*2), randint(-image_size, image_size*2)
        sx, sy = randint(-image_size, image_size*2), randint(-image_size, image_size*2)
        width = randint(1, 20)
...
def render_image(kanji, font, fsize, angle, image_size: int):
    img = Image.new("RGBA", (image_size, image_size), WHITE)

    bg = render_clutter(image_size)
    img.alpha_composite(bg)
    jitter = 3

    img_k = render_kanji(kanji, font, fsize, angle, image_size)
    x = np.clip(image_size//2 - img_k.width//2 + int(jitter*(random()*2-1)), 0, None)
    y = np.clip(image_size//2 - img_k.height//2 + int(jitter*(random()*2-1)), 0, None)
    img.alpha_composite(img_k, (x, y))

    return img


def gen_training_sample(all_kanji, all_fonts, image_size: int):
    ...
    img = render_image(kanji, font, fsize, angle + angle_noise, image_size)
